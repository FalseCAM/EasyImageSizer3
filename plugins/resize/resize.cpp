...
void Resize::convert(EisImage *image){
    ...
    if(ui->SizePercentRadioButton->isChecked()){
        image->setImage(image->getImage().scaled(image->getImage().width()*ui->SizeXPercentSpinBox->value()/100,
                                               image->getImage().height()*ui->SizeYPercentSpinBox->value()/100,
                                               Qt::IgnoreAspectRatio ));
    }else if(ui->SizePixelRadioButton->isChecked()){
        image->setImage(image->getImage().scaled(ui->SizeXPixelSpinBox->value(), ui->SizeYPixelSpinBox->value(),
                                               Qt::IgnoreAspectRatio));
    }else{}
    emit progress(100);
}
